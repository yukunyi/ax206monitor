...
func getRealCPUTemperature() float64 {
	if runtime.GOOS == "linux" {
		if temp := getLinuxCPUTemperature(); temp > 0 {
			return temp
		}
	}
...
func getRealCPUFrequency() (float64, float64) {
	...
	if runtime.GOOS == "linux" {
		if current, maxFreq := getLinuxCPUFrequency(); maxFreq > 0 {
			return current, maxFreq
		}
	}
...
func getDiskTemperature() float64 {
	if runtime.GOOS == "linux" {
		if temp := getLinuxDiskTemperature(); temp > 0 {
			return temp
		}
	}
...
func getNetworkInfo() NetworkInfoData {
	...
	if runtime.GOOS == "linux" {
		fallback := getLinuxNetworkInfo()
		if info.IP == "" {
			info.IP = fallback.IP
		}
		if info.UploadSpeed == 0 {
			info.UploadSpeed = fallback.UploadSpeed
		}
		if info.DownloadSpeed == 0 {
			info.DownloadSpeed = fallback.DownloadSpeed
		}
	}
